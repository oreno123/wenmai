...
export type ProceduralPatternType = 'huiwen' | 'wanzi' | 'binglie' | 'leiwen' | 'shengwen';
...
function createSVG(size: number, bgColor: string = 'transparent'): SVGSVGElement {
  const svg = svgEl('svg', {
...
function spiralHuiwenPath(startX: number, startY: number, size: number, turns: number): string {
  let x = startX
  let y = startY
  let w = size
  let h = size
...
function wanziPath(cx: number, cy: number, r: number): string {
  const t = r * 0.3 // 臂宽
  return [
    // 中心十字
    `M ${cx - t} ${cy - t}`,
...
export const PROCEDURAL_GENERATORS: Record<ProceduralPatternType, GeneratorConfig> = {
  huiwen: {
    name: '回纹',
 ...
 */
export function generatePatternSVG(type: ProceduralPatternType, params: Record<string, number> = {}): string {
 ...
 * 生成纹样并转为 data URL
 */
export function generatePatternDataURL(type: ProceduralPatternType, params: Record<string, number> = {}): string {
