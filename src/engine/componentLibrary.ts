...
interface CenterComponent extends BaseComponent {
  type: 'center';
}

interface BorderComponent extends BaseComponent {
  type: 'border';
  fitAngle: number;
}

interface CornerComponent extends BaseComponent {
  type: 'corner';
  position: string;
}

export type Component = CenterComponent | BorderComponent | CornerComponent;

// ── Internal Utility Functions ────────────────────────

function arc(cx: number, cy: number, r: number, startDeg: number, endDeg: number, segments: number = 32): string {
 ...
 */
export function getComponentSVG(
  component: Component,
 ...
 * 获取组件的 data URL
 */
export function getComponentDataURL(
  component: Component,
  stroke: string = '#C9A84C',
 ...
 */
export function drawComponentOnCanvas(
