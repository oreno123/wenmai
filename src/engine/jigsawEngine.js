 ...
 *    0    → straight (border)
 */
function generateEdge(x1, y1, x2, y2, sign) {
  if (sign === 0) {
 ...
 */
export function traceEdge(ctx, commands) {
 ...
 */
export function pieceToSVGPath(piece) {
 ...
 */
export function generatePuzzle(imgW, imgH, rows, cols, seed = 42) {
  const rng = seededRandom(seed)
  const cellW = imgW / cols
  const cellH = imgH / rows

  // Horizontal edge signatures: (rows+1) x cols
  // +1 → tab protrudes from row above INTO row below
  // Border edges are always 0
  const hEdges = []
  ...
  // +1 → tab protrudes from column left INTO column right
  const vEdges = []
  for (let r = 0; r < rows; r++) {
    vEdges[r] = []
    for (let c = 0; c <= cols; c++) {
 ...
 * @param {Object} piece    Piece definition from generatePuzzle
 * @param {HTMLImageElement} img  Source image (already sized to imgW x imgH)
 ...
 */
export function drawPiece(ctx, piece, img, x, y, scale = 1, isSelected = false, isSnapped = false) {
 ...
 *
 * @returns {{ snapX, snapY, targetId, distance }|null}
 */
export function findSnap(piece, px, py, placements, pieces, threshold = 45) {
 ...
 */
export function hitTestPiece(piece, x, y, px, py, scale = 1) {
  const lx = (px - x) / scale
  const ly = (py - y) / scale
 ...
 */
export function generateThumbnail(piece, img, size = 56) {
  const pad = piece.boundsPad
